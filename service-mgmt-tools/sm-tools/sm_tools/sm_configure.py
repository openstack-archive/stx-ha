        ...
        if args.which == "system":
            ...
            if args.sm_process_priority:
                if(int(args.sm_process_priority) in range(-20, -1)):
                    configure_system_opt("sm_process_priority", args.sm_process_priority)
                else:
                    print "Invalid sm_process_priority value. Must be in -2 to -20"
                    sys.exit(-1)
        else:
...
def configure_system_opt(key, value):
    ...
    sql = 'INSERT OR REPLACE INTO CONFIGURATION ( ID, "KEY", "VALUE" ) ' \
          'VALUES((SELECT ID FROM CONFIGURATION WHERE KEY = "%s"), "%s", "%s");' % \
          (key, key, value)
