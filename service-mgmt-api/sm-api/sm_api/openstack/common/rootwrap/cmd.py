...
"""Root wrapper for OpenStack services
   ...
   To use this with sm_api, you should set the following in
   sm_api.conf:
   rootwrap_config=/etc/sm_api/rootwrap.conf

   You also need to let the sm_api user run sm_api-rootwrap
   as root in sudoers:
   sm_api ALL = (root) NOPASSWD: /usr/bin/sm_api-rootwrap
                                   /etc/sm_api/rootwrap.conf *
...
def main():
    ...
    possible_topdir = os.path.normpath(os.path.join(os.path.abspath(execname),
                                                    os.pardir, os.pardir))
    if os.path.exists(os.path.join(possible_topdir, "sm_api", "__init__.py")):
