...
// ======================================
SmErrorT sm_service_group_go_standby_state_entry(
    SmServiceGroupT* service_group )
{
    SmErrorT error;

    error = sm_service_group_notification_notify(
                    service_group, SM_SERVICE_GROUP_NOTIFICATION_GO_STANDBY );
    if( SM_OKAY != error )
    {
...
// =====================================
SmErrorT sm_service_group_go_standby_state_exit( 
    SmServiceGroupT* service_group )
{
    SmErrorT error;
...
// ===========================================
SmErrorT sm_service_group_go_standby_state_transition(
    ...
    void* event_data[] )
{
    ...
    switch( event )
    {
        ...
        case SM_SERVICE_GROUP_EVENT_NOTIFICATION_TIMEOUT:
            ...
            if( SM_OKAY != error )
            {
                DPRINTFE( "Failed to go-standby on service group (%s), "
                          "error=%s", service_group->name,
                          sm_error_str( error ) );
                return( error );
            }
...
// ===========================================
SmErrorT sm_service_group_go_standby_state_initialize( void )
{
    return( SM_OKAY );
}
// ****************************************************************************
...
// =========================================
SmErrorT sm_service_group_go_standby_state_finalize( void )
{
    return( SM_OKAY );
}
// ****************************************************************************
