...
// ****************************************************************************
// Service Disable - Result Handler
// ================================
static SmErrorT service_disable_result_handler( SmServiceT* service,
...
// ****************************************************************************
// Service Disable - Timeout
// =========================
static bool sm_service_disable_timeout( SmTimerIdT timer_id, int64_t user_data )
{
    ...
    if( SM_OKAY != error )
    {
        DPRINTFE( "Failed to handle disable result for service (%s), "
                  "error=%s.", service->name, sm_error_str( error ) );
        abort();
    }
...
// ****************************************************************************
// Service Disable - Complete
// ==========================
static void sm_service_disable_complete( pid_t pid, int exit_code,
    int64_t user_data )
{
    SmServiceT* service;    
    SmServiceActionT action_running;
    ...
    if( SM_SERVICE_ACTION_DISABLE != service->action_running )
    {
        DPRINTFE( "Service (%s) not running disable action.", service->name );
        return;
    }
...
// Service Disable
// ===============
SmErrorT sm_service_disable( SmServiceT* service )
{
    char timer_name[80] = "";
    int process_id = -1;
    int timeout = 0;
    ...
    if( service->disable_check_dependency && !service->disable_skip_dependent )
    {
        // Are disable dependencies met?
        error =  sm_service_dependency_disable_met( service, &dependency_met );
        if( SM_OKAY != error )
        {
        ...
        DPRINTFD( "All dependencies for service (%s) were met.",
                  service->name );
    } else {
        DPRINTFD( "Dependency check for service (%s) skipped.",
                  service->name );
    }
    ...
    if( SM_OKAY != error )
    {
        DPRINTFE( "Failed to run disable action for service (%s), "
                  "error=%s.", service->name, sm_error_str( error ) );
        return( error );
    }
    ...
    error = sm_process_death_register( process_id, true,
                                       sm_service_disable_complete, 0 );
    if( SM_OKAY != error )
    {
        DPRINTFE( "Failed to register for disable action completion for "
    ...
    snprintf( timer_name, sizeof(timer_name), "%s %s action",
              service->name, sm_service_action_str( SM_SERVICE_ACTION_DISABLE ) );

    error = sm_timer_register( timer_name, timeout, sm_service_disable_timeout,
                               service->id, &timer_id );
    if( SM_OKAY != error )
    {
        DPRINTFE( "Failed to create a timer for action (%s) for service (%s), "
                  "error=%s.", sm_service_action_str( SM_SERVICE_ACTION_DISABLE ),
                  service->name, sm_error_str( error ) );
        abort();
    }

    // Write to database that we are running an action.
    service->action_running = SM_SERVICE_ACTION_DISABLE;
    service->action_pid = process_id;
    service->action_timer_id = timer_id;
    service->action_attempts += 1;

    DPRINTFI( "Started disable action (%i) for service (%s), flag (%d).", process_id,
              service->name, service->disable_skip_dependent );
...
// =======================
SmErrorT sm_service_disable_abort( SmServiceT* service )
{
    SmErrorT error;
...
// ========================
SmErrorT sm_service_disable_exists( SmServiceT* service, bool* exists )
{
    SmErrorT error;

    error = sm_service_action_exist( service->name, SM_SERVICE_ACTION_DISABLE );
...
// ============================
SmErrorT sm_service_disable_initialize( void )
{
    return( SM_OKAY );
}
// ****************************************************************************
...
// ==========================
SmErrorT sm_service_disable_finalize( void )
{
    return( SM_OKAY );
}
// ****************************************************************************
