...
// The is temporary to accommodate batching i/f state changes
// during the failover_audit for concurrent multiple failures
// handling. The same mechanism is implemented in
// fail_pending_state.
// In the end, the failover_audit will simply pump the events
// to the failover_fsm when it is received.
...
SmErrorT sm_failover_set_system(const SmSystemFailoverStatus& failover_status)
{
    ...
    if(SM_NODE_STATE_ACTIVE == host_target_state)
    {
            ...
            SM_NODE_STATE_FAILED == peer_target_state)
        {
            ...
            {
                DPRINTFE("Failed to active %s.", _host_name);
                return SM_FAILED;
            }
