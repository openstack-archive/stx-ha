...
static SmTimerIdT _query_timer_id = SM_TIMER_ID_INVALID;

#define LOOPBACK_IP "127.0.0.1"
#define SM_CLIENT_PORT_KEY "sm_client_port"
#define SM_SERVER_PORT_KEY "sm_server_port"
#define SM_CLUSTER_INFO_INTERVAL_KEY "cluster_info_query_interval_sec"

#define CLUSTER_INFO_QUERY_INTERVAL_MS 10000 // 10 seconds
...
unsigned short SmClusterHbsInfoMsg::_last_reqid = Invalid_Req_Id;
...
void SmClusterHbsInfoMsg::_cluster_hbs_info_msg_received( int selobj, int64_t user_data )
{
    ...
    if(Invalid_Req_Id == _last_reqid)
    {
        DPRINTFE("Runtime error, receive msg but no request sent");
        char buffer[100];
        while(0 < recv( selobj, &buffer, sizeof(buffer), MSG_NOSIGNAL | MSG_DONTWAIT ) );
        return;
    }
    ...
    while(true)
    {
        ...
        if(msg.reqid != _last_reqid)
        {
            DPRINTFI("Unmatched reqid, expected %d, received %d", _last_reqid, msg.reqid);
            continue;
        }

        _last_reqid = Invalid_Req_Id;
...
bool SmClusterHbsInfoMsg::cluster_hbs_info_query(SmTimerIdT timer_id, int64_t user_data)
{
    return cluster_hbs_info_query();
}
...
bool SmClusterHbsInfoMsg::cluster_hbs_info_query(cluster_hbs_query_ready_callback callback)
{
    ...
    }else
    {
        ...
        reqid = (*v) % 0xFFFE + 1; //leaving 0 as invalid_req_id
    }
...
SmErrorT SmClusterHbsInfoMsg::initialize()
{
    ...
    char cluster_info_query_interval_sce[SM_CONFIGURATION_VALUE_MAX_CHAR + 1];
    int interval_ms = 0;
    if( SM_OKAY != sm_configuration_table_get(SM_CLUSTER_INFO_INTERVAL_KEY,
            cluster_info_query_interval_sce, sizeof(cluster_info_query_interval_sce) - 1) )
    {
        DPRINTFE("Runtime error: system configuration %s undefined, using default %d ms",
                    SM_CLUSTER_INFO_INTERVAL_KEY,
                    CLUSTER_INFO_QUERY_INTERVAL_MS);
    }else
    {
        interval_ms = atoi(cluster_info_query_interval_sce) * 1000;
    }

    if(interval_ms <= 0)
    {
        interval_ms = CLUSTER_INFO_QUERY_INTERVAL_MS;
    }

    DPRINTFI("hbs cluster info query interval %d ms", interval_ms);
    error = sm_timer_register( "cluster info query timer",
                               interval_ms,
                               cluster_hbs_info_query, 0,
                               &_query_timer_id);
    if(SM_OKAY != error)
    {
        DPRINTFE("Failed to register timer");
        return SM_FAILED;
    }
    return SM_OKAY;
}
...
SmErrorT SmClusterHbsInfoMsg::finalize()
{
    SmErrorT error;
    mutex_holder holder(&_mutex);
    error = sm_timer_deregister(_query_timer_id);
    if(SM_OKAY != error)
    {
        DPRINTFE("Failed to deregister timer");
        return SM_FAILED;
    }
    _query_timer_id = SM_TIMER_ID_INVALID;
    if(_sock > 0)
    {
        close(_sock);
        _sock = -1;
