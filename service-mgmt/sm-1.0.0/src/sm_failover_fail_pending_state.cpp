...
static void disable_peer(SmSimpleAction&)
{
    ...
    int counter = 0;
    bool done = false;
    DPRINTFI("Blind guess to send mtce api to disable peer");
    ...
    while(!done && counter < 5)
    {
        ...
        if(SM_OKAY != error)
        {
            int wait_interval_us = 10000000; // 1 second
...
static bool wait_for_reset_timeout(SmTimerIdT timer_id, int64_t user_data)
{
    ...
    DPRINTFI("Blind guess stayfailed flag file is removed");
...
static bool reset_standby_timeout(SmTimerIdT timer_id, int64_t user_data)
{
    ...
    DPRINTFI("Blind guess stayfailed flag file is set");
    ...
    DPRINTFI("Blind guess wait for %d sce...", (WAIT_RESET_TIMEOUT + WAIT_RESET_TIMEOUT) / 1000);
    return false;
}
...
SmErrorT blind_guess_start_active()
{
    ...
    DPRINTFI("Blind guess wait for %d sce...", RESET_TIMEOUT / 1000);
    return SM_OKAY;
}
...
static void blind_guess_start_standby()
{
    SmErrorT error;
    sm_failover_utils_set_stayfailed_flag();
    DPRINTFI("Blind guess set stayfailed flag file");
    ...
    DPRINTFI("Blind guess wait for %d sec", (WAIT_RESET_TIMEOUT + GO_ACTIVE_TIMEOUT) / 1000);
}

void blind_guess_scenario_start()
{
    DPRINTFI("Blind guess starts...");
    ...
    DPRINTFI("Blind guess pre failure host state %s, peer state %s",
