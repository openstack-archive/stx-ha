...
// ==================================
SmErrorT sm_service_group_active_state_entry( SmServiceGroupT* service_group )
{
    SmErrorT error;

    error = sm_service_group_notification_notify(
                    service_group, SM_SERVICE_GROUP_NOTIFICATION_ACTIVE );
    if( SM_OKAY != error )
    {
    ...
    if( SM_OKAY != error )
    {
        DPRINTFE( "Failed to go-active on service group (%s), error=%s",
                  service_group->name, sm_error_str( error ) );
        return( error );
    }
...
// =================================
SmErrorT sm_service_group_active_state_exit( SmServiceGroupT* service_group )
{
    SmErrorT error;
...
// =======================================
SmErrorT sm_service_group_active_state_transition(
...
// ==========================================
SmErrorT sm_service_group_active_state_event_handler(
...
// =======================================
SmErrorT sm_service_group_active_state_initialize( void )
{
    return( SM_OKAY );
}
// ****************************************************************************
...
// =====================================
SmErrorT sm_service_group_active_state_finalize( void )
{
    return( SM_OKAY );
}
// ****************************************************************************
