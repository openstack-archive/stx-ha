...
// ****************************************************************************
// Service Enabled Active State - Audit Timer
// ==========================================
static bool sm_service_enabled_active_state_audit_timer( SmTimerIdT timer_id,
    int64_t user_data )
{
...
// ====================================
SmErrorT sm_service_enabled_active_state_entry( SmServiceT* service )
{
    char timer_name[80] = "";
    int audit_interval;
    ...
    if( service->audit_enabled_exists )
    {
        ...
        snprintf( timer_name, sizeof(timer_name), "%s enabled-active audit",
                  service->name );

        error = sm_timer_register( timer_name, audit_interval,
                                   sm_service_enabled_active_state_audit_timer,
...
// ===================================
SmErrorT sm_service_enabled_active_state_exit( SmServiceT* service )
{
    SmErrorT error;
...
// ============================================
SmErrorT sm_service_enabled_active_state_event_handler( SmServiceT* service,
    SmServiceEventT event, void* event_data[] )
{
    ...
    switch( event )
    {
        ...
        case SM_SERVICE_EVENT_HEARTBEAT_FAIL:
            ...
            {
                state = SM_SERVICE_STATE_ENABLED_GO_STANDBY;

            } else if( service->disable_action_exists ) {
                state = SM_SERVICE_STATE_DISABLING;

            } else {
                state = SM_SERVICE_STATE_NIL;
            }

            if( SM_SERVICE_STATE_NIL != state )
            {
        ...
        case SM_SERVICE_EVENT_DISABLE:
            ...
            {
                DPRINTFI( "Service (%s) received disable event without "
                          "desired action, event is ignored.", service->name );
            }
            break;
        case SM_SERVICE_EVENT_GO_STANDBY:
            ...
            {
                DPRINTFI( "Service (%s) received go_standby event without "
                          "desired action, event is ignored.", service->name );
            }
        break;
...
// =========================================
SmErrorT sm_service_enabled_active_state_initialize( void )
{
    return( SM_OKAY );
}
// ****************************************************************************
...
// =======================================
SmErrorT sm_service_enabled_active_state_finalize( void )
{
    return( SM_OKAY );
}
// ****************************************************************************
